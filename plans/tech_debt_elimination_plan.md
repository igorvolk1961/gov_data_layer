...
### TD-4: Исправить зависание интеграционных тестов
**Суть:** Интеграционные тесты зависают. Причина неизвестна.
**Решение:**
1. Добавить timeout на каждый тест (pytest-timeout)
2. Проверить deadlock в DatabaseClient (незакрытые соединения)
3. Проверить asyncio event loop — не блокируется ли на thread pool
**Оценка сложности:** Средняя
**Зависимости:** Нет
...
### TD-22: bge-m3 модель
**Суть:** 2.2 GB, медленная на CPU.
**Митигация:** Использовать `paraphrase-multilingual-MiniLM-L12-v2` (384 dim, 470 MB) по умолчанию. bge-m3 опционально.
