...
│   ├── service.py               # ★ ODLService — единый core-класс со всей бизнес-логикой
...
#### Шаг 4.1: Создать ODLService (`core/service.py`)
- Класс с 4 методами: `search_documents`, `get_document_detail`, `list_topics`, `get_toc`
- Интеграция с Tracer (observability)
- Преобразование `OfficialDocument` → `DocumentDetail`
- Обработка ошибок (проброс из адаптеров)

#### Шаг 4.2: Реализовать Router (`core/router/__init__.py`)
- Выбор адаптера по source_id
- Агрегация результатов из нескольких адаптеров
- Graceful degradation при недоступности адаптера

#### Шаг 4.3: Реализовать MCP-сервер (`core/api/mcp_server.py`)
...
#### Шаг 4.4: Реализовать OpenAPI-сервер (`core/api/rest_server.py`)
- FastAPI приложение
- Endpoints:
  - `GET /health`
...
- Автоматическая OpenAPI-документация (Swagger UI) через FastAPI

#### Шаг 4.5: Обновить точку входа (`core/main.py`)
- Создание ODLService с адаптерами
- Запуск MCP + REST параллельно
- Graceful shutdown
...
- `tests/integration/test_api.py` — интеграционные тесты (MCP + REST через StubAdapter)
...
- [ ] `ODLService.search_documents()` работает через StubAdapter
- [ ] `ODLService.get_document_detail()` работает через StubAdapter
...
- [ ] Оба сервера используют один и тот же `ODLService` (проверка через тест)
