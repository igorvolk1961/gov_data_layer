...
- [ ] Redis-кэш — только заглушка (`core/cache/__init__.py`)
- [ ] Graceful degradation — частично (адаптеры обёрнуты в try/except, но нет кэша)

### Фаза 6: Agent Skill и финальная полировка ❌
- [ ] `examples/SKILL.md` — существует, но требует обновления под актуальные модели
- [ ] Интеграционные тесты (сквозной сценарий)
- [ ] README — существует, но может требовать обновления

### Фаза 7: Адаптер pravo.gov.ru ❌
- [ ] HTTP-клиент для pravo.gov.ru
...
### Фаза 6: Пример Agent Skill и финальная полировка ❌
...
### Фаза 7: Первый адаптер — pravo.gov.ru ❌
...
| 5. Кэш + наблюдаемость | 🔄 Частично | Tracer готов, кэш — заглушка |
| 6. Agent Skill + полировка | ❌ Не начато | SKILL.md требует обновления |
| 7. Адаптер pravo.gov.ru | ❌ Не начато | |
