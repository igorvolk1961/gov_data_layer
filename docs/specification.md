...
`SearchContext` — структурированный контракт для фильтрации и Metadata Routing запроса. Передаётся вторым параметром в инструменты MCP (первый — `query`, свободный текст вопроса/интент пользователя).
...
> `official_only` удалён — слой по определению работает только с официальными источниками (см. раздел 1). Фильтрация «только официальное» — политика агента, а не механизм слоя.
...
- `get_document_detail`: до 4000 токенов
