...
Полный пайплайн с реальным источником publication.pravo.gov.ru не удобен для отладки, тестирования и демонстрации:
- Многие текущие правовые акты только вносят изменения или отменяют другие акты
- Разработка парсера документов, выявляющего внесение изменений или отмены - не тривиальная задача, выходящая за рамки PoC   
...
- **Только production** — отклонено: высокий риск при демонстрации, не готовность парсера документов
...
## 24. not_actual_since: фильтрация неактуальных чанков
...
## 25. Привязка чанков к рубрикам: per-chunk поиск
...
Рубрики секции — это объединение рубрик её чанков (агрегация на уровне `section_topic` в БД). `topic` удалён из API (`SearchContext`, `SearchRequest`).
...
## 26. Ранжирование: комбинированная формула с рубриками

**Дата:** 2026-07-21

### Проблема

После перехода на per-chunk поиск рубрик все чанки одного документа получают похожий набор `topic_ids`, так как документ тематически однороден. Фильтрация по `topic_ids` не разделяет чанки по релевантности — все проходят фильтр. Нужен способ ранжировать чанки внутри результата с учётом того, насколько тема чанка соответствует запросу.

### Решение

Использовать комбинированную формулу, учитывающую три сигнала:

```python
score = 0.6 * S1 + 0.4 * sqrt(max(S2 * S3))
```

Где:
- `S1` — косинусная схожесть вектора запроса с вектором чанка (query↔chunk)
- `S2` — косинусная схожесть вектора запроса с вектором рубрики (query↔topic)
- `S3` — косинусная схожесть вектора чанка с вектором рубрики (chunk↔topic), сохранённая в `topic_scores` при индексации

Логика:
- **`S1`** — основной сигнал: насколько текст чанка семантически похож на запрос
- **`max(S2 * S3)`** — перекрёстный сигнал: насколько запрос и чанк оба релевантны одной и той же рубрике (произведение даёт высокий вес только когда обе схожести высоки)
- **Квадратный корень** — увеличивает перекрёстный сигнал (произведение score < 1 делает его меньше, sqrt возвращает к масштабу S1)
- **Веса 0.6 / 0.4** — эмпирические, S1 важнее (это прямой поиск)

### Рассмотренные альтернативы

1. **Только S1** — отклонено: все прошедшие фильтр чанки имеют одинаковую тему, S1 плохо разделяет
2. **Только S2 * S3** (перекрёстный сигнал) — отклонено: игнорирует прямой textual similarity запроса с чанком
3. **S1 + S2 + S3** (линейная комбинация) — отклонено: S2 и S3 частично коррелируют, нет сжатия
4. **S1 * S2 * S3** (мультипликативная) — отклонено: слишком агрессивно, один низкий сигнал обнуляет результат

### Последствия

- Чанки ранжируются точнее: учитывается не только текст чанка, но и его тематическая близость к запросу
- `topic_scores` обязательны в payload Qdrant (S3 сигнал)
- Дополнительный `search_topics()` при поиске для получения S2
- Веса подобраны эмпирически, могут быть изменены при A/B-тестировании
