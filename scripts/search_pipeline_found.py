"""Search pipeline — demo: query that SHOULD find results.

Usage:
    uv run python scripts/search_pipeline_found.py                        # agent output
    uv run python scripts/search_pipeline_found.py --format human         # readable
    uv run python scripts/search_pipeline_found.py --format agent         # what agent gets
    uv run python scripts/search_pipeline_found.py --input               # JSON request body for REST API
"""
...
# ── Query ──────────────────────────────────────────────────────────────
# Document 0001202012230060: child benefits payment rules
...
def _format_human(response: SearchResponse, _response_model: dict[str, Any]) -> None:
    ...
    if response.results:
        print(f"\nFound {response.total_count} result(s):\n")
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description="Search pipeline demo: query that SHOULD find results"
    )
    parser.add_argument(
        "--format",
        choices=["human", "agent"],
    ...
    with tracer.trace("search_pipeline_found", query=QUERY[:100]) as root_span:
