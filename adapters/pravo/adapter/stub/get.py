"""StubGetHandler — stub document retrieval from fixed data."""

from __future__ import annotations

from typing import TYPE_CHECKING

from adapters.pravo.adapter.handlers import BaseGetHandler
from adapters.pravo.adapter.stub._data import _build_stub_documents
from core.errors import NotFoundError
from core.models.models import OfficialDocument

if TYPE_CHECKING:
    from adapters.pravo.adapter.base import PravoAdapterBase


class StubGetHandler(BaseGetHandler):
    """Retrieve a single document from fixed stub data."""

    def __init__(self, adapter: PravoAdapterBase) -> None:
        """Initialize with stub documents."""
        super().__init__(adapter)
        self._stub_documents = _build_stub_documents()

    async def get(self, document_id: str) -> OfficialDocument:
        """Look up a document by ID in stub data.

        Args:
            document_id: Document identifier.

        Returns:
            Document if found.

        Raises:
            NotFoundError: Document not found.
        """
        adapter = self._adapter
        with adapter.tracer.trace(
            "pravo.get",
            source_id=adapter.source_id,
            mode="stub",
            document_id=document_id,
        ) as span:
            span.set_input({"document_id": document_id})
            doc = self._stub_documents.get(document_id)
            if doc is None:
                span.set_error(NotFoundError(f"Document '{document_id}' not found in pravo stub"))
                raise NotFoundError(f"Document '{document_id}' not found in pravo stub")
            span.set_output({"id": doc.id, "title": doc.title})
            return doc


__all__ = [
    "StubGetHandler",
]
